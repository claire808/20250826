...
const V=video,O=overlay,CTX=O.getContext('2d');
...
(function(){
  ...
  bty.onclick=()=>{S.beautyOn=!S.beautyOn;bty.classList.toggle('on',S.beautyOn); V.style.filter=S.beautyOn?'brightness(1.06) contrast(1.04) saturate(1.05)':'none'};
...
async function startCam(){
  ...
  V.srcObject=stream; await V.play(); resize();
}
$('flipBtn').onclick=async()=>{S.facingMode=S.facingMode==='user'?'environment':'user'; await startCam();};
...
function pat(style,base){const p=document.createElement('canvas');p.width=128;p.height=64;const c=p.getContext('2d');
  switch(style){
...
function drawBand(rect){const {x,y,w,h,angle}=rect;CTX.save();CTX.translate(x,y);CTX.rotate(angle);
  ...
  const patC=pat(S.styleId,S.bandColor); CTX.fillStyle=CTX.createPattern(patC,'repeat'); CTX.globalAlpha=.88; rr(CTX,-w/2,-h/2,w,h,Math.min(h/2,22)); CTX.fill(); CTX.globalAlpha=1;
  CTX.fillStyle='#fff'; CTX.strokeStyle='rgba(0,0,0,.25)'; CTX.lineWidth=4; CTX.font=`${S.textWeight} ${S.textSize}px Inter, system-ui, -apple-system, 'Noto Sans TC', 'PingFang TC'`; CTX.textAlign='center';
...
function drawB(){for(let i=bubbles.length-1;i>=0;i--){const b=bubbles[i]; b.x+=b.vx;b.y+=b.vy;b.a-=.012;b.r*=1.003; if(b.a<=0){bubbles.splice(i,1);continue;}
  ...
  CTX.restore(); }}
// Loop
function render(){CTX.clearRect(0,0,O.width,O.height);
  if(results&&results.multiFaceLandmarks&&results.multiFaceLandmarks.length){const lm=results.multiFaceLandmarks[0]; const L=px(lm[127]),R=px(lm[356]),C={x:(L.x+R.x)/2,y:(L.y+R.y)/2}; const w=Math.hypot(R.x-L.x,R.y-L.y); const ang=Math.atan2(R.y-L.y,R.x-L.x); const bw=w*1.55,bh=Math.max(28,w*.22),off=-w*.22;
    const rect={x:C.x+Math.cos(ang-Math.PI/2)*(-off),y:C.y+Math.sin(ang-Math.PI/2)*(-off),w:bw,h:bh,angle:ang}; drawBand(rect);
    const top=px(lm[13]),bot=px(lm[14]); const ratio=Math.hypot(top.x-bot.x,top.y-bot.y)/w; const now=Date.now(); if(S.bubblesOn&&ratio>0.065&&(now-lastB)>110){spawn((top.x+bot.x)/2,(top.y+bot.y)/2); lastB=now;}
  }
...
share.onclick=async()=>{try{const res=await fetch(img.src); const blob=await res.blob(); const file=new File([blob],'headband.png',{type:'image/png'}); if(navigator.canShare&&navigator.canShare({files:[file]})) await navigator.share({files:[file],title:'Headband'}); else alert('此瀏覽器不支援原生分享，請改用「下載圖片」。')}catch(e){alert('分享失敗，請先下載圖片。')}};
// Boot
(async()=>{await startCam(); V.style.filter=S.beautyOn?'brightness(1.06) contrast(1.04) saturate(1.05)':'none'; tickFM(); render();})();
